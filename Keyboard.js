...
const Keyboard = (props) => {
  ...
  const keyPressed = (e) => {
    if (!keyIsHeld) {
      ...
      );//егер кнопка басылса symbol дын ышынен ыздейды
      if (foundSymbol) {
    ...
    if ("activeElement" in document) document.activeElement.blur();// htmlды озгертип жатр
  };

  const keyReleased = () => {
    setKeyPress("");// сакталад
    setkeyIsHeld(false);
  };
      ...
      {symbols.map((symbol) => {
        return (
          <button
            ...
            id={symbol.name}
            key={symbol.name}// корсетет барин
